...
import tool.ToolCloseOrderFuture;
import util.StringUtil;

/**
 * Servlet implementation class CloseOrderFutureServlet
 */
public class CloseOrderFutureServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CloseOrderFutureServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (LockedAuthorizedTokenLogic login = new LockedAuthorizedTokenLogic()) {
			...
			List<String> lines = new ToolCloseOrderFuture(X_API_KEY).execute(side, qty, price, expire);
