...
import tool.ToolCloseOrderMargin;
import util.StringUtil;

/**
 * Servlet implementation class CloseOrderMarginServlet
 */
public class CloseOrderMarginServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CloseOrderMarginServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (LockedAuthorizedTokenLogic login = new LockedAuthorizedTokenLogic()) {
			...
			List<String> lines = new ToolCloseOrderMargin(X_API_KEY).execute(symbol, side, qty, price, expire);
