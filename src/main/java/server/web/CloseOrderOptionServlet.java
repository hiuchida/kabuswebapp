...
import tool.ToolCloseOrderOption;
import util.StringUtil;

/**
 * Servlet implementation class CloseOrderOptionServlet
 */
public class CloseOrderOptionServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CloseOrderOptionServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (LockedAuthorizedTokenLogic login = new LockedAuthorizedTokenLogic()) {
			...
			List<String> lines = new ToolCloseOrderOption(X_API_KEY).execute(symbol, side, qty, price, expire);
