...
import tool.ToolCloseOrderStock;
import util.StringUtil;

/**
 * Servlet implementation class CloseOrderStockServlet
 */
public class CloseOrderStockServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CloseOrderStockServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (LockedAuthorizedTokenLogic login = new LockedAuthorizedTokenLogic()) {
			...
			List<String> lines = new ToolCloseOrderStock(X_API_KEY).execute(symbol, side, qty, price, expire);
