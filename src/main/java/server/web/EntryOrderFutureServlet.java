...
import tool.ToolEntryOrderFuture;
import util.StringUtil;

/**
 * Servlet implementation class EntryOrderFutureServlet
 */
public class EntryOrderFutureServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EntryOrderFutureServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (LockedAuthorizedTokenLogic login = new LockedAuthorizedTokenLogic()) {
			...
			List<String> lines = new ToolEntryOrderFuture(X_API_KEY).execute(side, qty, price, expire);
