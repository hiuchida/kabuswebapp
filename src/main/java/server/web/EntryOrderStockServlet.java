...
import tool.ToolEntryOrderStock;
import util.StringUtil;

/**
 * Servlet implementation class EntryOrderStockServlet
 */
public class EntryOrderStockServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EntryOrderStockServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (LockedAuthorizedTokenLogic login = new LockedAuthorizedTokenLogic()) {
			...
			List<String> lines = new ToolEntryOrderStock(X_API_KEY).execute(symbol, side, qty, price, expire);
