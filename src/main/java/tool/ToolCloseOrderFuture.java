 ...
 * 先物を返済注文するツール。
 */
public class ToolCloseOrderFuture extends AppCommon {
	/**
	 * クラス。
	 */
...
//		init(clazz);
		...
		try (LockedAuthorizedTokenLogic login = new LockedAuthorizedTokenLogic()) {
			...
			new ToolCloseOrderFuture(X_API_KEY).execute(side, qty, price, expire);
		}
	}
	 ...
	 */
	public ToolCloseOrderFuture(String X_API_KEY) {
