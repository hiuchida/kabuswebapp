 ...
 * 信用株式を新規注文するツール。
 */
public class ToolEntryOrderMargin extends AppCommon {
	/**
	 * クラス。
	 */
...
//		init(clazz);
		...
		try (LockedAuthorizedTokenLogic login = new LockedAuthorizedTokenLogic()) {
			...
			new ToolEntryOrderMargin(X_API_KEY).execute(symbol, side, qty, price, expire);
		}
	}
	 ...
	 */
	public ToolEntryOrderMargin(String X_API_KEY) {
	...
	public List<String> execute(String symbol, SideCode side, int qty, double price, int expire) throws ApiException {
		...
		req.setSide(side);
		req.setCashMargin(CashmarginStockCode.新規);
	...
	public List<String> executeV1(String symbol, SideCode side, int qty, double price, int expire) throws ApiException {
		...
		body.setCashMargin(CashmarginStockCode.新規.intValue());
